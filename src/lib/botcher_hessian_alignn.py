...
################################################################################
def eval_hess_vec_prod(vec, params, net, data_loader, loss_func, use_cuda=False):
    """
    ...
    for batch in data_loader:
        ...
        pred_outputs = pred_outputs + tuple([net((batch[0], batch[1])).expand(1)])
...
def min_max_hessian_eigs(net, data_loader, loss_func, rank=0, use_cuda=False, verbose=False, all_params=True):
    ...
    def hess_vec_prod(vec):
        ...
        eval_hess_vec_prod(vec, params, net, data_loader, loss_func, use_cuda)
